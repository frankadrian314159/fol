...
6.0$\times$--7.2$\times$ speedups and reduces per-iteration allocation to zero,
...
\begin{enumerate}
  ...
  shapes---the technique yields 6.0$\times$--7.2$\times$ speedups at 5M
...
what does it cost when disabled or inapplicable? All measurements use SBCL
2.6.0 on a single core; we report the mean of five runs after one warm-up run
and a full collection, with per-iteration allocation measured from the
runtime's bytes-consed counter. The three benchmarks all instantiate the
loop-carried record accumulator but differ in shape: a \emph{particle}
simulation (two independent additive field updates, the pattern of
Figure~\ref{fig:overview}), a 2D \emph{rotation} orbit (two fields updated by a
coupled, multiplicative map), and a three-field \emph{ballistic} integrator
(three fields, where the position update reads the freshly computed velocity).
...
\emph{no} constructor call on the back-edge. Per-iteration heap allocation drops
from one record per iteration to zero (the \texttt{Alloc/iter} column of
Table~\ref{tab:speedup}); the only surviving allocation is the single re-box at
loop exit. We confirm the transformation is applied by inspecting the generated
code for the scalar loop variables and the absence of the inlined callee on the
fast path.
...
6.0$\times$--7.2$\times$; the three-field integrator benefits most, as it conses
the largest record. Crucially, every optimized program returns results
bit-identical to its unoptimized counterpart; a dedicated test suite checks
optimized-equals-unoptimized on several record-accumulator shapes (field read at
exit, bare re-box at exit, inlined and non-inlined reconstructions), and the
full compiler test suite of 3{,}285 checks passes with the pass enabled.

\begin{table}
  \caption{Three allocation-bound benchmarks at 5M iterations (mean of five
  runs, SBCL 2.6.0, single core). Aggregate scalar replacement drives
  per-iteration allocation to zero; every result is bit-identical to its
  baseline.}
  \label{tab:speedup}
  \begin{tabular}{lrrrr}
    \toprule
    ...
    \midrule
    Particle \{x, y\}      & 1950 & 297 & 224\,B $\to$ 0 & 6.6$\times$ \\
    Rotation \{re, im\}    & 2799 & 466 & 224\,B $\to$ 0 & 6.0$\times$ \\
    Ballistic \{x, y, vy\} & 2554 & 357 & 256\,B $\to$ 0 & \textbf{7.2$\times$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
churn into flat scalar arithmetic for 6.0$\times$--7.2$\times$ speedups with
