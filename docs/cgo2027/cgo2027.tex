...
as a whole runs 90--290$\times$ slower than an equivalent native mutable-struct
...
speedup---the allocation share of the 90--290$\times$ gap to native---with
...
Our original four benchmarks focused on physics simulation patterns. To demonstrate
the breadth of the technique, we add four more from different domains: a
\textbf{Mandelbrot} set fractal (image processing), a 2D \textbf{Kalman Filter}
(state estimation), a \textbf{Biquad Filter} (audio DSP), \textbf{Streaming
Co-moments} (online statistics), and a \textbf{Lorenz} chaotic ODE integrator
(dynamical systems). The Lorenz benchmark deliberately uses a temporary-free
Euler integrator to isolate the accumulator's allocation cost, which ASR targets,
from body-local allocations that are outside its scope. Because a speedup over the
...
25--106\,ms (up to 4.4\,GB and 84 collections for the multi-accumulator Kalman
filter), whereas the scalar-replaced version conses \emph{nothing} and
triggers no workload collection at all. Per-iteration allocation drops from one
record (224 to 320\,B), or two records (448 to 512\,B) in the multi-accumulator
cases, to zero;
...
5.6$\times$--7.4$\times$. Crucially, every optimized program returns results
...
\begin{table}
  \caption{Nine allocation-bound benchmarks: wall time (mean $\pm$ sample
  stddev over five timed trials, after one warm-up) for the persistent baseline,
  aggregate scalar replacement (ASR), and a native mutable-\texttt{defstruct}
  upper bound; AMD Ryzen 5 7430U, SBCL 2.6.0. Iteration counts are chosen to
  give runtimes on the order of seconds. ASR drives per-iteration allocation
  (224--512\,B) to zero and is bit-identical to the baseline; the ``$\times$''
  columns are speedups over the baseline. The last four benchmarks are new additions
  from different domains. Two-body and Kalman Filter carry \emph{two} coupled
  record accumulators, both unboxed by the fixpoint analysis. Lorenz uses a 3-field record.}
  \label{tab:speedup}
  \small
  \begin{tabular}{lrrrrr}
    \toprule
    Benchmark (Iterations) & Base & ASR & Native & ASR & Native \\
              & (ms) & (ms) & (ms) & $\times$ & $\times$ \\
    \midrule
    Particle (5M)  & $1908\pm46$ & $294\pm1$  & 15 & 6.5 & 128 \\
    Rotation (5M)  & $2745\pm76$ & $492\pm11$ & 30 & 5.6 & \phantom{0}93 \\
    Ballistic (5M) & $2514\pm45$ & $371\pm3$  & 15 & 6.8 & 168 \\
    Two-body (5M)  & $6951\pm41$ & $964\pm5$  & 24 & 7.2 & 294 \\
    \midrule
    Mandelbrot (1M) & $3412\pm81$ & $585\pm9$ & 35 & 5.8 & \phantom{0}97 \\
    Kalman Filter (5M) & $8834\pm106$ & $1191\pm13$ & 48 & 7.4 & 184 \\
    Biquad Filter (10M) & $4105\pm62$ & $633\pm4$ & 41 & 6.5 & 100 \\
    Co-moments (10M) & $3550\pm55$ & $521\pm6$ & 29 & 6.8 & 122 \\
    Lorenz (5M) & $3115\pm51$ & $489\pm7$ & 22 & 6.4 & 142 \\
    \bottomrule
  \end{tabular}
\end{table}
...
worth attacking in the first place. Aggregate scalar replacement recovers the \emph{allocation}
component of that gap: it removes 100\% of per-iteration consing, worth the
5.6$\times$--7.4$\times$ of RQ2. It does \emph{not} close the gap
entirely---the scalar-replaced loop remains 15$\times$--41$\times$ slower than
native, reaching 2--7\% of native speed---and we report that rather than eliding
it. The gap is widest on the multi-accumulator benchmarks (Two-body, Kalman
Filter), whose heavier per-step arithmetic makes the generic-operator overhead
(below) proportionally larger. The residual is not allocation, of which none
remains, but arithmetic: the unboxed loop still calls FOL's \emph{generic}
operators (\texttt{+}, \texttt{*}, \texttt{-}) on its scalar variables, whereas
the native loop issues declared single-float machine instructions. Closing it is
a matter of scalar type specialization, which is orthogonal to allocation and in
fact complementary to this transformation---scalar replacement is what exposes
the per-field variables that a type specializer would then annotate, and the two
factors compose multiplicatively toward native. Attacking allocation first is
not arbitrary: it is the factor that produces garbage-collection pauses (the
baseline triggers 20--84 collections per call; ASR triggers none), so it removes
GC from the loop entirely---a latency benefit, not merely the throughput factor
the speedup measures---and it is the prerequisite for the arithmetic work. As an
allocation optimization, ASR is complete on these benchmarks; the remaining
distance is a separate, larger axis of work.
...
validity: FOL is a research vehicle without an outside user base, and our four
