...
inapplicable. On an allocation-bound particle simulation the technique delivers
a 5.8$\times$ speedup and reduces per-iteration allocation to zero, with
byte-identical results and no cost when disabled.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Evaluation.} On an allocation-bound particle simulation the
  technique yields a 5.8$\times$ speedup at 5M iterations, reduces per-iteration
  allocation to zero, produces byte-identical results to the unoptimized
  program, and adds no cost when disabled or inapplicable
  (\S\ref{sec:eval}).
\end{enumerate}
...
Our evaluation is deliberately narrow: this technique targets one high-value
pattern, and we report an honest picture of when it fires and what it buys.
...
what does it cost when disabled or inapplicable? All measurements use SBCL on a
single core.

\textbf{RQ1: Firing and allocation.} On the particle simulation of
Figure~\ref{fig:overview}, the pass fires: the emitted fast path
(Figure~\ref{fig:emitted}) splits \texttt{p} into \texttt{p\_x}/\texttt{p\_y},
inlines \texttt{update-point}, and contains \emph{no} constructor call on the
back-edge. Per-iteration heap allocation of \texttt{<point>} objects drops from
one per iteration to zero; the only surviving allocation is the single re-box at
...
\textbf{RQ2: Speedup and correctness.} Table~\ref{tab:speedup} reports the
particle simulation at five million iterations. Eliminating allocation removes
the garbage-collection pressure that dominates the baseline, yielding a
5.8$\times$ speedup. Crucially, the optimized and unoptimized programs return
identical results; a dedicated test suite checks optimized-equals-unoptimized
on several record-accumulator shapes (field read at exit, bare re-box at exit,
inlined and non-inlined reconstructions), and the full compiler test suite of
3{,}285 checks passes with the pass enabled.

\begin{table}
  \caption{Particle simulation (5M iterations): eliminating per-iteration
  allocation removes the GC bottleneck.}
  \label{tab:speedup}
  \begin{tabular}{lrrr}
    \toprule
    Configuration & Time (ms) & Alloc/iter & Speedup \\
    \midrule
    Baseline (persistent objects) & 4123 & 1 object & 1.0$\times$ \\
    Aggregate scalar replacement  &  719 & 0 objects & \textbf{5.8$\times$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
and declines the rest. On an allocation-bound simulation it turns object
churn into flat scalar arithmetic for a 5.8$\times$ speedup with identical
results. We see it as the record-shaped complement to automatic transient
