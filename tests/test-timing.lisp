...
(test multi-pattern-dispatch-timing
  ...
  (let* ((env (make-standard-module)))
    ...
    (format t "100000 iterations with 1-arg pattern:~%")
    (fol-eval (fol.reader:fol-read-from-string "(%time% (dotimes [_ 100000] (process 0)))") env)
    (format t "100000 iterations with predicate pattern (< 10):~%")
    (fol-eval (fol.reader:fol-read-from-string "(%time% (dotimes [_ 100000] (process 0 5)))") env)
    (format t "100000 iterations with type pattern (number string):~%")
    (fol-eval (fol.reader:fol-read-from-string "(%time% (dotimes [_ 100000] (process 0 0 \"abc\")))") env)
    (format t "100000 iterations with fallback pattern:~%")
    (fol-eval (fol.reader:fol-read-from-string "(%time% (dotimes [_ 100000] (process 0 0 0)))") env)
    (pass)))
