...
Short-lived intermediates are reclaimed in SBCL's nursery, keeping GC overhead at 1.5\% in the LSim benchmark; the $14{\times}$ peak allocation in the AST benchmark (Section~\ref{sec:ast-bench}) reflects this path-copying volume but remains bounded by the trie depth.
