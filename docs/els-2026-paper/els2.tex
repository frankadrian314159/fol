...
The contributions compose: Listing~\ref{lst:predicate-persistent} (Section~\ref{sec:dispatch}) shows a predicate specializer gating on a runtime value while reading persistent slot state---neither contribution alone suffices. Listing~\ref{lst:guard} shows $O(1)$ update discard enabled by the pre-update snapshot being an immutable value; the \texttt{:around} qualifier and the immutability invariant are both required for the pattern to work without copying.\footnote{FOL addresses the \emph{new-operations} dimension of the Expression Problem~\cite{Wadler1990}: new generic functions can be defined over existing classes without modification; adding a new data type with distinct behavior may require new method clauses.}
...
\noindent At first post-redefinition \emph{mutation} (\texttt{assoc}/\texttt{dissoc}), chain replay finds \texttt{:reading} in discarded slot property-lists and writes it into \texttt{:val} in the new object. Before that mutation, reads of \texttt{:reading} resolve via the alias map at the call site; no instance modification occurs on a read-only access.
...
2-hop (v1$\to$v3) & 3.57 & 5.62 & CL plist scan grows $O(A{\cdot}H)$; FOL alias-map composition is $O(A)$ regardless of hop depth \\
...
At extreme scale, fragmentation in the CL mutable heap degrades cache locality; the persistent heap's $O(\log n)$ path-copying avoids unbounded fragmentation, yielding the 3.8$\times$ advantage.
