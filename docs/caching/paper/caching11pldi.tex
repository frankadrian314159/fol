...
Edge-case analysis suggests: **selective caching for expensive predicates**. Components: (1)~cost instrumentation (>1~µs threshold); (2)~selective enablement (expensive predicates only); (3)~adaptive toggling (disable if hit rate <95\%); (4)~per-thread caching (eliminates 88× lock contention).
...
**The Optimization Paradox**: The constraint solver is caching's strongest case—at 5.3~µs baseline, caching should save 6~µs. Yet V8 optimizes it to 6.69~ns via inlining and constant folding (0.00× ratio). V8 recognizes the loop result depends only on input and optimizes away the entire computation.

**Design-space implication**: Expensive-predicate condition (>1~µs cost) theoretically enables break-even. However, V8 shows optimizers detect expensive operations and specialize them away, eliminating caching viability. Languages with expensive-predicate dispatch would have compilers defeat caching via specialization.
