...
%% Title information
\title{WhyObject-Level Dispatch Caching Fails Universally}
...
\begin{abstract}
  ...
  We implement and benchmark polymorphic caching for multi-argument dispatch across twenty
  distinct language implementations: compiled natives (SBCL, CCL, LispWorks, Chez, Go), bytecode JIT
  (ABCL, C2, GraalVM), compiled JIT (Dart), tracing JIT (V8, LuaJIT, PyPy), optional types (Typed Racket, TypeScript), and interpreted
  ...
  (LuaJIT: 84× slower, V8: $\infty \times$ slower). Analysis reveals three universal failure modes: (1)~caching
...
caching strategies. However, these successes rely on \emph{machine-code inline caching}: JIT
compilers embed type checks and direct jumps into compiled code. A natural question arises: can
\emph{object-level caching}—storing dispatch information in hash tables without JIT
infrastructure—achieve the same benefits?

This paper presents a comprehensive empirical study addressing this question across twenty
distinct language implementations spanning six language families, combined with testing of five
...
Testing across twenty implementations and five dispatch mechanisms reveals:
\begin{itemize}
  \item \textbf{19/20 implementations}: Clear failure (>1.1× slowdown)
  \item \textbf{1/20 implementations}: Marginal benefit (1.02× range)
  \item \textbf{0/20 implementations}: Clear speedup
...
Failures span three orders of magnitude:
\begin{itemize}
  \item Marginal (1.15× slowdown in Go)
  \item Severe (5.3× slowdown in SBCL, 84× in LuaJIT)
  \item Catastrophic (V8, C2 with $\infty$ slowdown from escape analysis)
\end{itemize}

\subsection{Contributions}

\begin{enumerate}
  \item A comprehensive empirical study of object-level dispatch caching across twenty diverse
    implementations (compiled natives, bytecode JITs, compiled JITs, tracing JITs, interpreted, optional types) and
  ...
  \item Validation of break-even theory through hash dispatch analysis: the sole mechanism showing
    marginal benefit (5% speedup) corresponds exactly to the predicted theoretical break-even point.
...
We implement consistent dispatch caching benchmarks across all twenty implementations, following
three scenarios:
...
\begin{enumerate}
  ...
  \item \textbf{Lock-free hash table}: Requires compare-and-swap (10--20~ns). Not faster
    than mutex; adds complexity. Alternative: persistent/structural-sharing hash tables enable
    lock-free reads (one CAS per write), but add memory overhead from creating new versions.

  \item \textbf{Per-thread cache}: Eliminates contention but multiplies memory (32$\times$ on
    32-core system). Also reduces hit rate when dispatch types are shared across threads.
\end{enumerate}
...
\begin{itemize}
  \item \textbf{Interactive/server workloads} (web/database): High concurrency (8--32 threads),
    significant contention. Overhead exceeds 50~ns per cache access.

  \item \textbf{Batch/scientific computing}: Low concurrency (1--4 threads).
    Minimal contention; 20--30~ns overhead (worse than uncached for fast dispatch).

  \item \textbf{Single-threaded languages} (JavaScript, Python): No lock contention;
    11--17~ns overhead (best-case, what we measure).
\end{itemize}
...
\begin{table*}[t!]
\centering
\caption{Dispatch caching performance across all 21 implementations (heterogeneous 4-type cycle).}
...
C2 (escape analysis baseline) & $<$5.0 & \infty & $\infty \times$ & Catastrophic \\
GraalVM (JDK 25.0.3) & 15.9 & 20.5 & 1.29× & Fail \\
\\
\textbf{Compiled JIT:} \\
Dart (Flutter 3.13+) & 23.3 & 237.7 & 10.18× & Severe \\
\\
\textbf{Tracing JIT:} \\
V8 (JavaScript) & $<$1.0 & \infty & $\infty \times$ & Catastrophic \\
Julia (1.10 compiled) & 68.4 & 246.2 & 3.60× & Fail \\
...
Mean cached & 1,735.1 & & & \\
Mean ratio & 18.3× & & & \\
Clear failures (>1.02×) & & & 20/21 & 95.2\% \\
Marginal ($\leq$1.02×) & & & 1/21 & 4.8\% \\
Clear speedups & & & 0/21 & 0.0\% \\
\bottomrule
\end{tabular}
\end{table*}
...
Dart (Flutter 3.13+) shows severe failure (10.18× slowdown) despite 99.9998\% cache hit rates,
revealing that closure allocation overhead (210~ns) dominates dispatch savings. Dart's uncached
dispatch (23.3~ns) is highly optimized, but the caching mechanism requires creating closure objects
for cached dispatch functions, adding memory allocation cost. The cached path (237.7~ns) represents
pure overhead: high hit rates provide no benefit because cache lookup cost + allocation far exceeds
uncached dispatch. This is Failure Mode 1 observed in a JIT setting: when baseline dispatch is
ultra-optimized, adding a cache layer with object allocation overhead becomes net-negative.
...
Cache lookup (memory access, hash computation, indirect calls) takes longer than the COND
dispatch it is meant to accelerate.

\subsubsection{Failure Mode 3: JIT Defeats Caching}
...
\midrule
V8 & $<$1.0 & N/A & $\infty \times$ (escape analysis) \\
C2 & $<$5.0 & N/A & $\infty \times$ (escape analysis) \\
...
Advanced JITs (escape analysis, per-site specialization) compile dispatch to near-zero cost.
Object-level caching cannot match this; it adds overhead instead.
...
\begin{enumerate}
  ...
  \item \textbf{16+ slots plateau}: Beyond the working set size, adding more slots provides no
    further benefit. All cache sizes $\geq$16 slots show 12--13× slowdown, converging toward the
...
\begin{enumerate}
  ...
  \item \textbf{Implementability}: Feasible to implement identically across 20 implementations
\end{enumerate}
...
LRU with timestamp & 18--25 & +3-5 ns; minimal benefit \\
Adaptive Replacement (ARC) & 25--35 & +11-15 ns; overhead outweighs benefit \\
LIRS & 30--40 & +16-20 ns; complex, high overhead \\
Clock algorithm & 16--22 & +2-4 ns; minimal improvement \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{itemize}
  \item \textbf{What it is}: Type switch on one parameter (simplest, most common pattern).
  \item \textbf{Example}: \texttt{(case (class-of x) (int ...) (string ...) (list ...))}
  \item \textbf{Baseline cost}: 1.6~ns (ultra-fast type check)
  \item \textbf{Practical impact}: Most damaging; fundamental to all OO programming (method dispatch on receiver).
...
\begin{itemize}
  \item \textbf{What it is}: Type switch on multiple parameters (double/multidispatch).
...
\begin{itemize}
  \item \textbf{What it is}: Predicate-based multimethod dispatch (tests multiple conditions sequentially).
  \item \textbf{Example}: Sequential predicate testing: \texttt{(if (pred1 x) ... (if (pred2 x) ...)...)}
  \item \textbf{Baseline cost}: 2.5~ns (ultra-fast predicate sequence)
  \item \textbf{Practical impact}: Advanced dispatch (Clojure, Julia, predicate-based multimethods).
  \item \textbf{Why caching fails}: Overhead (16--20~ns) is 6--8× baseline. Worst case in study.
\end{itemize}

\textbf{4. Property-Based Dispatch} (15.63× slowdown):
\begin{itemize}
  \item \textbf{What it is}: Protocol/trait membership checking (not type-based, structure-based).
  \item \textbf{Example}: Clojure protocols, Rust traits, Python duck typing.
  \item \textbf{Baseline cost}: 1.3~ns (fast type assertions)
  \item \textbf{Practical impact}: Fundamental to protocol-based languages (structural dispatch).
  \item \textbf{Why caching fails}: Same overhead dominance as single-argument (1500\% of baseline).
\end{itemize}

\textbf{5. Hash Dispatch} (5% speedup):
\begin{itemize}
...
Paradoxically, simplest dispatch (single-arg, generic function) fails worst because:
...
\begin{itemize}
  \item \textbf{HHVM} (PHP JIT): Likely failure (1.3--2× slowdown), like GraalVM and OpenJDK C2
...
Our contribution is empirical scope: no prior work benchmarked dispatch caching across 20
...
\begin{enumerate}
  \item \textbf{Hit rate $\neq$ Performance}: 99.99\%+ cache hit rates coincide with worst performance
...
\begin{enumerate}
  ...
  \item \textbf{Invest in machine-code caching}. JIT-based caching works (V8, PyPy, GraalVM).
...
caching universally fails across all dynamic languages tested (20 implementations spanning 6
...
For researchers and practitioners: Do not implement object-level dispatch caching. Invest instead in:
(1)~machine-code caching (JIT-based), (2)~optimized dispatch compilation, (3)~per-site specialization.
Object-level caching is a trap: high hit rates but low efficiency, worse than no caching.
