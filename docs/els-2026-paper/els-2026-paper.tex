...
\midrule
\multicolumn{3}{l}{\textit{Method combination logic and allocation follow standard CLOS behavior (precedence follows $\prec_{\text{disp}}$).}} \\
\bottomrule
\end{tabularx}
\end{table*}
...
\textit{s-clause}& ::= & \texttt{[} \textit{pattern}^* \texttt{]} ~ \textit{body}^+ \\
\textit{m-clause}& ::= & \texttt{\{} (\textit{symbol} ~ \textit{pattern} \mid \texttt{:keys~[} (\textit{symbol} \mid \texttt{(}s \textit{p}\texttt{)})^* \texttt{]})^* \texttt{\}} ~ \textit{body}^+ \\
...
Functional updates are 33--447$\times$ slower than mutation. Naive Quicksort and BFS (Listings~\ref{lst:naive-qsort}--\ref{lst:naive-bfs}) illustrate the path-copying penalty (Table~\ref{tab:adversarial-bench}); Quicksort 100k incurring $90\times$ overhead. Transients recover performance by batching these updates.
...
$N=50{,}000$    & FOL persistent (lazy dict)      &  200\,ms &  61\,MB   & 27$\times$ \\
                & FOL transient (\texttt{hamt-assoc!}) &   94\,ms &  16\,MB   & 13$\times$ \\
\bottomrule
\end{tabular}
\end{table}

Merge sort overhead ($34\times$) stems from $O(\log N)$ trie node touches ($1.7M$ calls). Lazy BFS ($27\times$) improves on naive BFS by eliminating initialization waste. Transients help BFS ($27\times \to 13\times$) but are counterproductive for Merge Sort ($34\times \to 37\times$): persistent vector \texttt{conj} already touches only a root-to-leaf path of depth $\le 5$ per append, so transient setup overhead is not recovered---it actively adds cost at this scale.
...
FOL is $19.2{\times}$ slower and uses $7.1{\times}$ more memory per optimization pass. The overhead has two sources. \textbf{Field reads} via the \texttt{get} generic function cost ${\approx}50$\,ns versus ${\approx}4$\,ns for a struct slot reference ($12{\times}$). \textbf{Object construction} via \texttt{make-<class>} costs ${\approx}183$\,ns versus ${\approx}37$\,ns for mutable \texttt{make-instance} ($5{\times}$); see Section~\ref{sec:micro-bench} for the \texttt{\%make-persistent} construction analysis.
...
The ratio peaks at 9.4$\times$ (8x32-900) and narrows to 6.9$\times$ at the largest configuration. Normalising by gate evaluations clarifies the mechanism: CL's per-gate simulation cost is flat at 1.4--2.6\,$\mu$s/eval across all six configurations, because the mutable leftist heap's working set remains bounded by $O(\log n)$ cons cells on the merge path---small enough to stay cache-resident regardless of circuit size. FOL starts at ${\approx}$28\,$\mu$s/eval for the smallest circuit and converges toward ${\approx}$12\,$\mu$s/eval at the two largest as the fixed $O(\log n)$ per-merge allocation cost is amortized over more simulation work per queue operation. GC accounts for only 2.0\% of FOL run time at large scale.
...
The ratio narrows from 9.4 to 6.9$\times$ as scale increases. Normalised by gate evaluations, FOL converges toward ${\approx}12\,\mu$s/eval, confirming that persistent heap overhead is bounded by the fixed $O(\log n)$ per-merge cost, not by circuit size.
...
\textbf{Persistent maps} impose 2.0--2.6$\times$ slowdown, while \textbf{persistent heap} overhead decreases with size (3.6$\times \to 1.9\times$) as the fixed $O(\log n)$ per-merge allocation cost is amortized over more simulation work per queue operation. At the largest scale (8$\times$32$\times$32-9000), the combined cost is approximately explained by the product of both overheads scaled by the $\approx$1.6$\times$ dispatch factor ($2.6 \times 1.9 \times 1.6 \approx 7.9\times$, vs.\ observed $7.8\times$); at smaller scales the interaction is sub-multiplicative, likely because persistent-map and persistent-heap allocation pressure overlap in the same nursery generation rather than adding independently.
...
Predicate dispatch was pioneered in Cecil~\cite{Chambers1993} and formalized as a unified theory by Ernst et al.~\cite{Ernst1998}; both require theorem-prover subsumption for method ordering. FOL sacrifices exactness for syntactic predictability. Filtered dispatch~\cite{Orleans2002} and singleton specializers in Dylan~\cite{Apple1992} also influenced our model. Slate~\cite{Salzman2005} extended multiple dispatch with prototype-based receivers; FOL instead preserves CLOS method combinations. Clojure's \texttt{defmulti}~\cite{Hickey2008} dispatches on the return value of a user-supplied function; method ordering is managed via \texttt{prefer-method} (see Section~\ref{sec:dispatch} for a detailed comparison). Prior work on CLOS persistence has typically mapped objects to external stores or serialized them to disk, incurring I/O costs absent in FOL's in-memory tries. FOL adopts the ``objects as maps'' philosophy of Self~\cite{Ungar1987} but on a functional substrate, and the epochal time model of Clojure~\cite{Hickey2009}.
...
Open problems include: native compilation to replace the $O(N)$ predicate scan with a static type-check sequence, static clause analysis to detect unreachable patterns at load time, and distribution of persistent value identities across nodes for coordinated concurrent state.
