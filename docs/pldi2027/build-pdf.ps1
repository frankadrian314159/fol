<#
.SYNOPSIS
    Compiles the PLDI 2027 LaTeX paper using MiKTeX.
.DESCRIPTION
    Runs the standard LaTeX build sequence (PDFLaTeX -> BibTeX -> PDFLaTeX x2)
    to resolve citations and references for the PLDI 2027 paper.
.PARAMETER FileName
    The name of the LaTeX file (without .tex extension) to compile. Defaults to "pldi2027".
#>

param(
    [string]$FileName = "pldi2027-2"
)
