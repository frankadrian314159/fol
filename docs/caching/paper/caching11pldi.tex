...
Hölzle et al.~\cite{hoelzle1991optimizing} introduced polymorphic inline caching for Smalltalk (2--4× speedups via machine-code type checks). Modern JITs (V8, PyPy, GraalVM) report similar benefits. Our work studies orthogonal approach: object-level caching (runtime data structures) vs. machine-code specialization.

\subsection{Adaptive Optimization and Dispatch Efficiency}

Chambers/Ungar~\cite{chambers1989optimizing} and Arnold/Ryder~\cite{arnold2005feedback} optimize via dynamic specialization (JIT-dependent). Campbell/Wolczko~\cite{campbell1992array} and Steele/Gabriel~\cite{steele1990lisp} show compiled dispatch achieves 1--100~ns (validating SBCL 30~ns). Escape analysis~\cite{kotzmann2005escape} eliminates allocation, making external caching redundant.
