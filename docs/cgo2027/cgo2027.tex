...
a whole runs 80--230$\times$ slower than an equivalent native mutable-struct
...
inapplicable. Across twelve allocation-bound benchmarks the technique eliminates
per-iteration allocation entirely for an end-to-end 2.8$\times$--15.2$\times$
speedup (the top of that range from one dispatch-bound benchmark, \S\ref{sec:eval})---the
allocation share of the 80--230$\times$ gap to native---with
...
objects, transpiled to Common Lisp---a loop that rebuilds a persistent record runs \textbf{80--230$\times$}
...
\begin{enumerate}
  ...
  \item \textbf{Evaluation.} On twelve allocation-bound benchmarks from five
  ...
  2.8$\times$--15.2$\times$ speedup (dominated by one dispatch-bound benchmark,
  \S\ref{sec:eval}; the rest span 2.8$\times$--6.2$\times$), produces byte-identical results, and adds no
...
loop-carried record accumulator---and we measure it on twelve microbenchmarks that instantiate
...
\textbf{RQ1: Firing and allocation.} The pass fires on all twelve benchmarks: each
...
costing 16--84\,ms---whereas the scalar-replaced version conses \emph{nothing}
...
2.8$\times$--15.2$\times$. One benchmark accounts for the top of that range on
...
emitting direct scalar arithmetic---so Assoc reaches 15.2$\times$, more than
double any other benchmark. Excluding it, the remaining eleven span a more
typical 2.8$\times$--6.2$\times$: the physics kernels (Particle, Rotation,
Ballistic, Two-body, Clamp), which are the most allocation-bound, see
4.4$\times$--6.2$\times$, while the arithmetic-heavier kernels (Mandelbrot,
Kalman, biquad, co-moments, Lorenz) see 2.8$\times$--4.2$\times$---the more a
loop computes per step, the smaller the share of its runtime that allocation
represents, and the smaller the win from removing it (RQ4). The Counter
benchmark, the simplest possible single-field case, sees a 6.1$\times$
speedup, in line with the other physics-family kernels. Crucially, every optimized program returns results
...
suite of 3{,}321 checks passes with the pass enabled.
...
\begin{table}
  ...
  first six are physics kernels; the next five are from other domains; Counter
  is the simplest possible single-field case. Two-body and Kalman carry two
  coupled record accumulators, both unboxed by the fixpoint. Assoc's outsized
  speedup comes from a dispatch-bound baseline, not an unusually fast optimized
  path (\S\ref{sec:eval}).}
  \label{tab:speedup}
  \small
  \begin{tabular}{lrrrrr}
    ...
    Benchmark & Base & ASR & Native & ASR & Native \\
              & (ms) & (ms) & (ms) & $\times$ & $\times$ \\
    \midrule
    Particle   & $3624\pm166$ & \phantom{0}$638\pm17$  & \phantom{0}33 & \phantom{0}5.7 & 111 \\
    Rotation   & $5246\pm177$ & \phantom{0}$992\pm14$  & \phantom{0}61 & \phantom{0}5.3 & \phantom{0}86 \\
    Ballistic  & $4699\pm42$\phantom{0}  & \phantom{0}$754\pm16$  & \phantom{0}33 & \phantom{0}6.2 & 144 \\
    Two-body   & $12038\pm246$ & $2067\pm75$\phantom{0} & \phantom{0}52 & \phantom{0}5.8 & 232 \\
    Clamp      & $4435\pm119$ & $1006\pm14$ & \phantom{0}36 & \phantom{0}4.4 & 125 \\
    Assoc      & $7504\pm204$ & \phantom{0}$494\pm17$  & \phantom{0}33 & \textbf{15.2} & 228 \\
    \midrule
    Mandelbrot & $4434\pm92$\phantom{0}  & $1266\pm39$ & \phantom{0}53 & \phantom{0}3.5 & \phantom{0}84 \\
    Kalman     & $10857\pm194$ & $3612\pm117$ & 110 & \phantom{0}3.0 & \phantom{0}99 \\
    Biquad     & $6400\pm63$\phantom{0}  & $1542\pm45$ & \phantom{0}56 & \phantom{0}4.2 & 114 \\
    Co-moments & $6760\pm74$\phantom{0}  & $1868\pm92$ & \phantom{0}70 & \phantom{0}3.6 & \phantom{0}96 \\
    Lorenz     & $6273\pm288$ & $2264\pm68$\phantom{0} & \phantom{0}64 & \phantom{0}2.8 & \phantom{0}99 \\
    \midrule
    Counter    & $2673\pm45$\phantom{0}  & \phantom{0}$437\pm15$  & \phantom{0}33 & \phantom{0}6.1 & \phantom{0}82 \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{RQ3: Distance to native.} A hand-written native version---a mutable
\texttt{defstruct} updated in place---is 82$\times$--232$\times$ faster than the
...
consing, worth the 2.8$\times$--15.2$\times$ of RQ2. It does \emph{not} close the
gap entirely---the scalar-replaced loop remains 13$\times$--40$\times$ slower
than native, reaching 3--7\% of native speed. The decomposition is consistent
across all twelve benchmarks: the allocation factor ASR removes (its speedup)
...
removal, and it moves the scalar-replaced loops from 3--7\% of native to
...
\texttt{case}-branched reconstructions are all recognized directly
(\emph{Clamp}, \S\ref{sec:eval}, exercises the \texttt{if} case), but only when
...
validity: FOL is a research vehicle without an outside user base, and our twelve
...
recognizes and declines the rest. Across twelve allocation-bound benchmarks it
eliminates per-iteration allocation entirely for 2.8$\times$--15.2$\times$
speedups (one dispatch-bound benchmark accounts for the top of that range;
the rest span 2.8$\times$--6.2$\times$) with identical results, recovering the allocation component of a
80--230$\times$ gap to native mutable structs. The remainder---a larger factor
