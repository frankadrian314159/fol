...
Benchmarks: 24 implementations, 200K warmup + 3 runs of 2M iterations. Per-language standard deviation ±0.5~ns; ratio CI ±0.5\% (all ratios >1.1× are statistically robust). JVM measurements retain escape analysis (production baseline). Cache keys: class tuples. AMD Ryzen 9, Windows 11. Single-threaded (multi-threading adds 2--5× contention, Section~\ref{sec:discussion}).
