...
Load-time decidable $\prec_{\text{disp}}$ conservatively approximates semantic specificity: Level-3 predicates fire before Level-2 types, even for superset extensions. Use explicit guards---\texttt{(and (<integer>) (even?))}---to clarify domains rather than relying on structural conventions alone.
...
FOL's validated guard avoids Clojure's wrappers (Listing~\ref{lst:guard-clj}) or standard Common Lisp's $O(N)$ copies (Listing~\ref{lst:guard-cl}).
...
\textbf{Construction} (${\approx}183$\,ns) is $5{\times}$ native; \texttt{\%make-persistent} bypasses initialization. \textbf{Reads} add 1.2--2.0$\times$ overhead; \textbf{Updates} are 30--246$\times$ slower.
$T=8$ is optimal for 8-slot objects and $N \ge 32$; it yields to higher thresholds only at intermediate widths rare in practice~\cite{Lanza2006}.
...
\textbf{Persistent maps} add 2.0--2.6$\times$; \textbf{heap} overhead falls from $3.6{\times}$ to $1.9{\times}$ at scale. Their interaction is sub-multiplicative as allocation pressure overlaps.
...
The AST-optimizer and LSim benchmarks exercise FOL's key value propositions.
We port an idiomatic CLOS expression interpreter (seven node types) to FOL with minimal changes.
...
The CL baseline uses mutable environments; FOL uses \texttt{(assoc env bvar v)} with structural sharing. Method bodies are otherwise identical.
...
\textbf{Experimenter bias}: FOL and all baselines share the same author; more aggressive CL tuning might narrow the reported ratios.
...
\textbf{Qualitative comparison}: Section~\ref{sec:dispatch} compares code-level conciseness but does not measure programmer productivity; a controlled user study remains future work.
\textbf{Interoperability hazard}: CL authors extending FOL classes may be surprised by runtime mutation errors. These restrictions follow from the persistence invariant but are not signaled during method definition.
...
Predicate dispatch~\cite{Chambers1993,Ernst1998} typically requires theorem-prover subsumption for method ordering. FOL favors a syntactic level ordering to avoid $O(N^2)$ subsumption costs, sacrificing semantic exactness for load-time decidability. JPred~\cite{Millstein2004} uses domain containment checks, but these are fragile in CLOS's open-world model. Our proposed composite level (Section~\ref{sec:dispatch}) aims to recover automatic ordering for type-annotated predicates without a theorem prover.
...
Dylan's singletons~\cite{Apple1992} and Slate's multiple dispatch~\cite{Salzman2005} are restricted forms of FOL's approach.
Clojure's \texttt{defmulti}~\cite{Hickey2008} dispatches via a user-supplied function and resolves ambiguity via \texttt{prefer-method}.
\textit{Protocols}~\cite{Hickey2010protocols} dispatch on the first argument's class only, with no predicate specialization.
FOL's \texttt{defmethod} shares \texttt{defmulti}'s niche---open, multi-parameter, extensible---but adds predicate specializers and CLOS method combinations, trading an $O(N)$ linear scan for increased expressivity.
