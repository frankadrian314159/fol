...
that the core rules handle, and name-resolution faithfulness is a property of how
`Σ` is constructed, not the rules themselves. The main theorem (proof sketched
in an appendix) states that if an accumulator qualifies, the transient-converted
program is observationally equivalent to the original, proven via bisimulation
against a store semantics.
...
\textbf{Inter-procedural Analysis Scope.} Our ``Tier-2'' summary inference
engine would automatically analyze user-defined functions. The core of this
engine would be the same uniqueness-at-death classifier, repurposed to produce a
usage summary for each of a function's parameters. To handle dependencies, the
analysis would operate on a file-at-a-time basis, first building a call graph of
all functions in the compilation unit. For recursive functions (including
mutually recursive groups identified within the call graph's strongly connected
components), the analysis would iterate to a fixpoint, starting with a
conservative summary and refining it until it stabilizes. The inferred summaries
would be attached to function metadata and, crucially, would be world-guarded:
if an optimized loop in function `f` depends on an inferred summary for `g`,
redefining `g` must invalidate the optimized region in `f`.
...
Finally, our transient representation for sets does not yet support reads,
limiting the optimization's applicability for that collection type.
