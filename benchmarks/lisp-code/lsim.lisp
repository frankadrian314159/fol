...
(defun insert-event (queue event)
  "Insert event into a time-sorted list iteratively to avoid stack overflow."
  (let ((time (sim-event-time event)))
    (if (or (null queue) (< time (sim-event-time (car queue))))
        (cons event queue)
        (let ((curr queue))
          (loop while (and (cdr curr)
                           (<= (sim-event-time (cadr curr)) time))
                do (setf curr (cdr curr)))
          (setf (cdr curr) (cons event (cdr curr)))
          queue))))

(defun merge-events (queue new-events)
  "Merge a list of new events into a sorted queue."
  (let ((q queue))
    (dolist (e new-events q)
      (setf q (insert-event q e)))))
