...
allocations: on our system a single field update on a persistent object is up
to 89$\times$ slower than the same update on a native mutable struct. Classic
...
inapplicable. Across three allocation-bound benchmarks the technique delivers
6.5$\times$--7.2$\times$ speedups and reduces per-iteration allocation to zero,
with byte-identical results and no cost when disabled.
\end{abstract}
...
Lisp---a field update on a persistent object is up to \textbf{89$\times$} slower
than the equivalent update on a native mutable \texttt{defstruct}, dominated by
...
\begin{enumerate}
  ...
  shapes---the technique yields 6.5$\times$--7.2$\times$ speedups at 5M
  iterations, reduces per-iteration allocation to zero, produces byte-identical
  results to the unoptimized program, and adds no cost when disabled or
  inapplicable (\S\ref{sec:eval}).
\end{enumerate}
...
We evaluate three questions. \textbf{RQ1:} does the transformation fire on the
...
what does it cost when disabled or inapplicable? All measurements were taken on
an AMD Ryzen 5 7430U (used single-threaded) with 32\,GB of RAM, running SBCL
2.6.0---generational GC, 1\,GB dynamic space, 51\,MB nursery---on Windows 11.
For each benchmark and configuration we run one warm-up call followed by five
timed calls, forcing a full collection before each, and report the mean $\pm$
sample standard deviation. Wall time uses the real-time clock; per-call
allocation, garbage-collection counts, and time spent in GC are read from the
SBCL runtime (the bytes-consed counter, an after-GC hook, and
\texttt{*gc-run-time*}); speedup is computed per paired trial and then averaged.
The three benchmarks all instantiate the loop-carried record accumulator but
differ in shape: a \emph{particle} simulation (two independent additive field
updates, the pattern of Figure~\ref{fig:overview}), a 2D \emph{rotation} orbit
(two fields updated by a coupled, multiplicative map), and a three-field
\emph{ballistic} integrator (three fields, where the position update reads the
freshly computed velocity).
...
roughly 20 collections costing 34--47\,ms, whereas the scalar-replaced version
conses \emph{nothing} and triggers no workload collection at all. Per-iteration
allocation drops from one record to zero (the \texttt{Alloc/iter} column of
Table~\ref{tab:speedup}); the only allocation that survives is the single re-box
at loop exit, executed at most once per activation.
...
garbage-collection pressure that dominates each baseline, yielding speedups of
6.5$\times$--7.2$\times$; the three-field integrator benefits most, as it conses
the largest record and also shows the tightest variance. Run-to-run variability
is modest---sample standard deviations are within 8\% of the mean and are
dominated by baseline GC jitter---and, being wall-clock on a desktop OS, it is
reported rather than smoothed away. Crucially, every optimized program returns
...
\begin{table}
  \caption{Three allocation-bound benchmarks at 5M iterations: mean $\pm$ sample
  standard deviation over five timed trials (after one warm-up), AMD Ryzen 5
  7430U, SBCL 2.6.0. Aggregate scalar replacement drives per-iteration
  allocation to zero; every result is bit-identical to its baseline.}
  \label{tab:speedup}
  \begin{tabular}{lrrrr}
    \toprule
    Benchmark & Base (ms) & ASR (ms) & Alloc/iter & Speedup \\
    \midrule
    Particle \{x, y\}      & $2121\pm78$  & $307\pm4$ & 224\,B $\to$ 0 & 6.9$\times$ \\
    Rotation \{re, im\}    & $3141\pm254$ & $486\pm8$ & 224\,B $\to$ 0 & 6.5$\times$ \\
    Ballistic \{x, y, vy\} & $2584\pm10$  & $361\pm4$ & 256\,B $\to$ 0 & \textbf{7.2$\times$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
and declines the rest. Across three allocation-bound benchmarks it turns object
churn into flat scalar arithmetic for 6.5$\times$--7.2$\times$ speedups with
identical results. We see it as the record-shaped complement to automatic transient
