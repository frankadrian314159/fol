...
\textbf{DVI does not convert---and why.} DVI accumulates through a user-defined helper, \texttt{(add-item cart price)}, called inside a four-binding \texttt{bind}, not at \texttt{cart}'s own recur or exit position: \texttt{add-item}'s result flows through \texttt{cart-total} (a further read) and one more \texttt{assoc} before the value that actually recurs is produced, two bindings later. Chain-through-bind (§\ref{sec:chains}) now folds a \emph{multi-step} chain across an entire \texttt{bind}, closing the general case, not just a single binding---but DVI's own shape still breaks it: the intervening \texttt{cart-total} binding \emph{reads} the running alias through a helper rather than extending the chain, exactly the kind of binding the mechanism deliberately treats as chain-breaking rather than silently skippable (§\ref{sec:chains}). We also close a second, independent obstacle: our transient protocol now has a representation for \texttt{<persistent-object>} instances generally (a shallow copy marked owned by the current thread, using the mutation machinery \texttt{assoc} on persistent objects already relies on), so \texttt{cart}'s CLOS representation is no longer itself a barrier.

We close the third obstacle too, without abandoning the black-box summary approach we originally judged it would require. \texttt{<cart>} defines a custom \texttt{:around} method on \texttt{assoc}---the "automatic derived-value invalidation" the benchmark is named for. \texttt{Assoc}'s own primary method for \texttt{<persistent-object>} already routes every update through \texttt{update-slot}, whose in-place fast path is keyed on thread ownership, not on which caller invoked it, so $\mathrm{Trusted}$ can instead route this accumulator's spine-op calls through the real \texttt{assoc} generic rather than the bang-op bypass: the customization's method combination fires exactly as CLOS defines it, while \texttt{call-next-method} still reaches the same ownership-aware primary---at a measured $\approx$10ns/call marginal dispatch cost over the bypass (isolated via microbenchmark: an identical mutation through a single-method generic vs.\ a primary-plus-\texttt{:around} pair), paid only by accumulators whose class is actually customized. Verified directly on \texttt{<cart>}'s own \texttt{:around} shape---\texttt{(slot (= :items))} guard included, the compiler bug that once collapsed such a clause's CLOS specializers to \texttt{T} now fixed (§\ref{sec:opgates})---conversion succeeds, and the customization's side effect provably still fires on the converted path (a sentinel value cleared only by the \texttt{:around}, not by initialization).

We close the fourth obstacle too: the intervening-read shape itself. Chain-through-bind's duplication-safety invariant required \emph{every} binding to extend the running alias; we generalize it to tolerate a binding that only \emph{reads} the alias, provided a Tier-1 or Tier-2-inferred summary proves the read never retains it (\texttt{\%safe-read-of-p}, reusing the same summary consultation \texttt{call-arg-read-p} already used elsewhere)---closing a companion gap in Tier-2 inference itself, which had never recognized a keyword-accessor call as non-retaining and so could never prove this for a helper like \texttt{cart-total} shaped exactly like it. Unlike chain-through-bind's fold-into-one-expression strategy, a read has no place in a folded expression, so this rebuilds the \texttt{bind} in place instead: chain-extending bindings are rewritten, the read is left completely untouched, and nothing is reordered or dropped---the concern that motivated the original restriction. Verified directly on \texttt{cart-total}'s own shape (a helper reading through \texttt{if}/keyword-accessor/\texttt{reduce}) composed with the \texttt{:around} obstacle above on the same accumulator.

All four catalogued obstacles are now closed, and \texttt{run-bench} converts for the first time. But converting is not the speedup we projected from the dispatch-cost microbenchmark alone: \texttt{cart}'s own two slots mutate in place, but \texttt{:items}---the FOL \texttt{<vector>} \emph{stored in} one of those slots---does not. \texttt{add-item}'s \texttt{(assoc cart :items (conj (get cart :items) price))} calls \texttt{conj} on the vector \emph{value} retrieved from the slot, an ordinary, fully separate persistent-vector operation our persistent-object mechanism has no visibility into: \texttt{transient} on a \texttt{<persistent-object>} marks the object owned, not values reachable through its slots. Measured on the full benchmark (N=1{,}000, R=1{,}000, same machine/protocol as §\ref{sec:eval}): converting moves DVI from 23.61$\times$ to 23.47$\times$ slower than hand-written CL, a near-noise wall-time difference, because 1{,}000 writes are a small fraction of the benchmark's 1{,}000{,}000 total operations and the write path itself still pays an uneliminated \texttt{conj} on every call. Extending transience to travel through a persistent object's own fields is a fifth, structurally distinct obstacle---invisible until the first four were closed enough for DVI to reach this code path at all---and remains future work.

Profiling \texttt{run-bench} directly located the dominant allocator: not the write-path \texttt{conj} above, but \texttt{cart-total}'s cache-miss fold, \texttt{(reduce + 0 (:items cart))}, at $\approx$90\% of total allocation---exposing a pre-existing \texttt{reduce} bug unrelated to transient conversion (it affects the unconverted path identically): \texttt{collection-seq} on a \texttt{<vector>} already had an O(1)-amortized iterator over its HAMT trie, but built and discarded a freshly-consed list from it on every call, an $O(n)$ allocation on every one of \texttt{cart-total}'s cache misses. A new \texttt{collection-reduce} generic folds \texttt{<vector>} directly over that iterator instead (default method unchanged for every other collection kind, regression-tested against it); \texttt{reduce} now delegates to it. This general fix, not transient conversion's own contribution, cuts DVI's allocation 8.3$\times$ (9.08\,MB to 1.10\,MB, unconverted path, before vs.\ after) and is why the numbers above are lower than an earlier measurement of this same benchmark; it does not change the fifth-obstacle conclusion, which is about the write path this fix leaves untouched.
