...
In FOL, the cost is stark: simple accumulation loops run up to 114× slower than handwritten, mutable Common Lisp (55× more memory). This paper presents a sound, automatic optimization eliminating the tax for a large class of \emph{intraprocedural} accumulation loops.
...
\textbf{A harder interprocedural accumulator---and why it barely converts.} A companion benchmark studied while developing this system threads an accumulator through a helper function inside a \texttt{bind}, with a customized \texttt{:around} method on \texttt{assoc} implementing automatic derived-value cache invalidation on top---e.g.\ a running total kept in one slot that must be cleared whenever another slot's underlying data changes, so a later read recomputes it instead of returning a stale value---a shape our classifier originally refused on four independent grounds: an interprocedural chain hidden behind the helper and a further intervening read, the accumulator's CLOS (not dict/vector/set) representation, and the customized method combination itself. We closed all four---extending chain-through-bind to fold multi-step chains and tolerate a non-retaining read (§\ref{sec:chains}), adding a \texttt{<persistent-object>} transient representation (§\ref{sec:edittagged}), and trusting \texttt{assoc}'s customization enough to route spine-op calls through the real generic instead of refusing outright, at a measured $\approx$10ns/call marginal cost, verified to actually fire the customization's side effect on the converted path---detailed mechanism-by-mechanism in Appendix~\ref{app:obstacles}. The accumulator now converts, but the speedup is a near-noise 23.61$\times\to$23.47$\times$ against hand-written CL: its 1,000 writes are a small fraction of 1,000,000 total operations, and a fifth, still-open obstacle---a nested persistent \texttt{<vector>} reachable from an owned slot is not itself made transient---leaves the write path's own allocation uneliminated. Profiling also exposed and fixed an unrelated \texttt{reduce} allocation bug, cutting this benchmark's allocation 8.3$\times$ independent of conversion (Appendix~\ref{app:obstacles}). This benchmark is not otherwise tracked in this paper's tables. A structurally different, more tractable variant of this same read-side cost is noted as future work (§\ref{sec:conclusion}).
...
\textbf{Interprocedural class/type inference for dynamic languages.} Tier-2 (§\ref{sec:tier2}) proves a narrow fact---that a call returns a fresh, unaliased value of a known representation kind---by a whole-compilation-unit fixed point over call sites, closing on redefinition. Three prior systems target a related fact under different soundness commitments. Class Hierarchy Analysis \cite{dean1995cha} narrows virtual dispatch statically from the declared type hierarchy rather than call-site provenance; it is sound but, in an open, redefinable world like FOL's, would need some invalidation mechanism on hierarchy change to stay so. Polymorphic inline caches \cite{holzle1991pic} instead observe types dynamically per call site and specialize opportunistically---adaptive where Tier-2 is conservative, but unsound without a deoptimization path our compile-time, world-guarded approach avoids needing. Furr et al.\ \cite{furr2009ruby} infer static types for Ruby via a whole-program constraint system over the full type lattice, a richer target than Tier-2's single \texttt{:returns-fresh-p} fact; our summaries stay narrower for a fixed point cheap enough to run on every compile. We have not measured Tier-2 against any of these three systems directly; §\ref{sec:eval}'s RQ3 evaluates it only on the pattern it was built to close.
...
\section{A Harder Interprocedural Accumulator: Mechanism by Mechanism}
\label{app:obstacles}

§\ref{sec:discussion} summarizes a companion benchmark that accumulates through a user-defined helper, \texttt{(update-derived acc price)}, called inside a four-binding \texttt{bind}, not at \texttt{acc}'s own recur or exit position: \texttt{update-derived}'s result flows through \texttt{derived-total} (a further read) and one more \texttt{assoc} before the value that actually recurs is produced, two bindings later. Chain-through-bind (§\ref{sec:chains}) folds a \emph{multi-step} chain across an entire \texttt{bind}, closing the general case, not just a single binding---but this benchmark's own shape still breaks it: the intervening \texttt{derived-total} binding \emph{reads} the running alias through a helper rather than extending the chain, exactly the kind of binding the mechanism deliberately treats as chain-breaking rather than silently skippable (§\ref{sec:chains}). A second, independent obstacle: our transient protocol needed a representation for \texttt{<persistent-object>} instances generally (a shallow copy marked owned by the current thread, using the mutation machinery \texttt{assoc} on persistent objects already relies on), so \texttt{acc}'s CLOS representation is no longer itself a barrier.

The third obstacle closes without abandoning the black-box summary approach we originally judged it would require. \texttt{acc}'s class defines a custom \texttt{:around} method on \texttt{assoc}---a derived-value cache invalidated automatically whenever the underlying data changes, the pattern that motivated this whole investigation. \texttt{Assoc}'s own primary method for \texttt{<persistent-object>} already routes every update through \texttt{update-slot}, whose in-place fast path is keyed on thread ownership, not on which caller invoked it, so $\mathrm{Trusted}$ can instead route this accumulator's spine-op calls through the real \texttt{assoc} generic rather than the bang-op bypass: the customization's method combination fires exactly as CLOS defines it, while \texttt{call-next-method} still reaches the same ownership-aware primary---at a measured $\approx$10ns/call marginal dispatch cost over the bypass (isolated via microbenchmark: an identical mutation through a single-method generic vs.\ a primary-plus-\texttt{:around} pair), paid only by accumulators whose class is actually customized. Verified directly on this \texttt{:around} shape---\texttt{(slot (= :data))} guard included, the compiler bug that once collapsed such a clause's CLOS specializers to \texttt{T} now fixed (§\ref{sec:opgates})---conversion succeeds, and the customization's side effect provably still fires on the converted path (a sentinel value cleared only by the \texttt{:around}, not by initialization).

The fourth obstacle is the intervening-read shape itself. Chain-through-bind's duplication-safety invariant required \emph{every} binding to extend the running alias; we generalize it to tolerate a binding that only \emph{reads} the alias, provided a Tier-1 or Tier-2-inferred summary proves the read never retains it (\texttt{\%safe-read-of-p}, reusing the same summary consultation \texttt{call-arg-read-p} already used elsewhere)---closing a companion gap in Tier-2 inference itself, which had never recognized a keyword-accessor call as non-retaining and so could never prove this for a helper like \texttt{derived-total} shaped exactly like it. Unlike chain-through-bind's fold-into-one-expression strategy, a read has no place in a folded expression, so this rebuilds the \texttt{bind} in place instead: chain-extending bindings are rewritten, the read is left completely untouched, and nothing is reordered or dropped---the concern that motivated the original restriction. Verified directly on \texttt{derived-total}'s own shape (a helper reading through \texttt{if}/keyword-accessor/\texttt{reduce}) composed with the \texttt{:around} obstacle above on the same accumulator.

All four catalogued obstacles closed, this benchmark's accumulation loop converts for the first time. But converting is not the speedup projected from the dispatch-cost microbenchmark alone: \texttt{acc}'s own two slots mutate in place, but \texttt{:data}---the FOL \texttt{<vector>} \emph{stored in} one of those slots---does not. \texttt{update-derived}'s \texttt{(assoc acc :data (conj (get acc :data) price))} calls \texttt{conj} on the vector \emph{value} retrieved from the slot, an ordinary, fully separate persistent-vector operation our persistent-object mechanism has no visibility into: \texttt{transient} on a \texttt{<persistent-object>} marks the object owned, not values reachable through its slots. Measured on the full benchmark (N=1{,}000, R=1{,}000, same machine/protocol as §\ref{sec:eval}): converting moves this benchmark from 23.61$\times$ to 23.47$\times$ slower than hand-written CL, a near-noise wall-time difference, because 1{,}000 writes are a small fraction of the benchmark's 1{,}000{,}000 total operations and the write path itself still pays an uneliminated \texttt{conj} on every call. Extending transience to travel through a persistent object's own fields is a fifth, structurally distinct obstacle---invisible until the first four were closed enough for this benchmark to reach this code path at all---and remains future work.

Profiling this benchmark's inner loop directly located the dominant allocator: not the write-path \texttt{conj} above, but \texttt{derived-total}'s cache-miss fold, \texttt{(reduce + 0 (:data acc))}, at $\approx$90\% of total allocation---exposing a pre-existing \texttt{reduce} bug unrelated to transient conversion (it affects the unconverted path identically): \texttt{collection-seq} on a \texttt{<vector>} already had an O(1)-amortized iterator over its HAMT trie, but built and discarded a freshly-consed list from it on every call, an $O(n)$ allocation on every one of \texttt{derived-total}'s cache misses. A new \texttt{collection-reduce} generic folds \texttt{<vector>} directly over that iterator instead (default method unchanged for every other collection kind, regression-tested against it); \texttt{reduce} now delegates to it. This general fix, not transient conversion's own contribution, cuts this benchmark's allocation 8.3$\times$ (9.08\,MB to 1.10\,MB, unconverted path, before vs.\ after); it does not change the fifth-obstacle conclusion, which is about the write path this fix leaves untouched.
