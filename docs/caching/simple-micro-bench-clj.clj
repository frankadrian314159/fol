#!/usr/bin/env clojure
;;; Micro-benchmark: homogeneous dispatch (Clojure version)
;;; All dispatches on numbers (longs) only
...
;; Cache variable
(def %-homo-cache (create-dispatch-cache))

;; Uncached dispatcher
(defn dispatch-homo-uncached [x]
  (cond
    (number? x) [:long x]
...
(defn dispatch-homo-cached [x]
  (let [key (cond
    ...
    (if hit
      (do
        (swap! %-homo-cache update :hits inc)
        (hit x))
      (do
        (swap! %-homo-cache update :misses inc)
        (cond
          (number? x)
          (do
            (cache-insert! %-homo-cache key %-clause-long)
            (%-clause-long x))
          (string? x)
          (do
...
;; Test data: 2,000,000 number-only calls (10x to get measurable time)
(def ^:dynamic *test-data*
  (vec (repeatedly 2000000 #(rand-int 1000000))))
    ...
    (if (>= run iterations)
      ...
      (let [result-box (volatile! 0)
        ...
        (dotimes [i (count *test-data*)]
          (let [r (dispatch-homo-uncached (nth *test-data* i))]
            (when r
...
(defn benchmark-cached [iterations]
  (loop [run 0 times []]
    (if (>= run iterations)
      times
      (do
        (reset! %-homo-cache (assoc @%-homo-cache :hits 0 :misses 0))
        (let [result-box (volatile! 0)
              start-ns (System/nanoTime)]
          (dotimes [i (count *test-data*)]
            (let [r (dispatch-homo-cached (nth *test-data* i))]
              (when r
                (vswap! result-box inc))))
          (let [elapsed-ms (/ (- (System/nanoTime) start-ns) 1000000.0)]
            (println (format "  Run %d: %,.1f seconds (result: %d)" (inc run) (/ elapsed-ms 1000.0) @result-box))
...
(defn run-all-benchmarks []
  (println "\n================================")
  (println "Clojure Homogeneous Dispatch Caching Micro-Benchmark")
  (println "================================")
  (println (format "Implementation: Clojure %s" (clojure-version)))
  (println "Test data: 2,000,000 number-only calls (10× iterations)\n")

  (println "Warming up JIT compiler (10,000 calls)...")
  (doseq [i (range 10000)]
    (dispatch-homo-uncached (nth *test-data* (mod i (count *test-data*)))))
  (doseq [i (range 10000)]
    (reset! %-homo-cache (assoc @%-homo-cache :hits 0 :misses 0))
    (dispatch-homo-cached (nth *test-data* (mod i (count *test-data*)))))
  ...
  (let [{:keys [hits misses]} @%-homo-cache
        total (+ hits misses)]
