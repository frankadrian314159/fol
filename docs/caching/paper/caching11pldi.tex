...
Benchmarks: 24 implementations, 200K warmup + 3 independent runs of 2M iterations each. **Statistical methodology**: Each run is independent (cache cleared between runs); per-language standard deviation computed across 3 runs ±0.5~ns; 95\% CI on ratios ±0.5\% relative (e.g., ±0.06× at 11.49×). All ratios >1.1× represent robust failures (>2 standard errors from break-even). **Note on Clojure (244.8×)**: Tight CI (±1.1\%) reflects consistent allocation pressure across runs; each run triggers 3--5× increased GC frequency, making behavior reproducible despite catastrophic slowdown. JVM measurements retain escape analysis (production baseline). Cache keys: class tuples. AMD Ryzen 9, Windows 11. Single-threaded (multi-threading adds 2--5× contention, Section~\ref{sec:discussion}).
...
Edge-case analysis suggests a viable approach: **selective caching for expensive predicates**. Required components: (1)~predicate cost instrumentation (profile >1~µs threshold); (2)~selective enablement (cache only expensive predicates, not all dispatch); (3)~adaptive toggling (disable if hit rate <95\%); (4)~per-thread isolation (thread-local caches eliminate 88× lock contention in SBCL).

**Applicability**: Languages with reflection and user-defined expensive predicates (Clojure multimethods, CLOS methods). Typed languages (Go, Java) show minimal benefit. Framework is domain-specific, not general-purpose.

\subsection{Per-Thread and Type-Biased Specialization}

**Per-thread caching**: SBCL achieves 3.1× vs. 88.94× shared (8 threads). Cost: 1--16~KB per thread (256~MB on 256-thread systems). Viable for long-running servers with stable thread pools (<100 threads); prohibitive for serverless/Lambda.
...
\begin{enumerate}
  ...
  \item \textbf{Type-based dispatch only}: Value-predicate or exotic dispatch might differ if predicates are expensive (>1~µs). Framework predicts similar failure, but exceptions possible.
...
\begin{itemize}
  \item Email/URL/Date regex (44--69~ns baseline): 0.23–0.14× (JIT specializes regex compilation to near-zero overhead)
  \item Constraint solver (5323~ns baseline): 0.00× (6.69~ns cached vs. 5323~ns uncached)
\end{itemize}

**The Optimization Paradox**: The constraint solver represents the strongest theoretic case for caching—at 5.3~µs baseline, caching should provide 6~µs savings per call. Yet V8 optimizes it to 6.69~ns, faster than cached dispatch (showing 0.00× ratio). This occurs via aggressive inlining, escape analysis, and constant folding: V8 recognizes that the loop's result depends only on the input value and optimizes the entire 500-iteration loop to a constant.

**Design-space implication**: The expensive-predicate condition (>1~µs cost) theoretically enables break-even. However, V8 suggests "expensive" predicates are compiler specialization opportunities, not caching candidates. Optimizers detect expensive operations and specialize them away, eliminating caching viability conditions. This validates universality: languages with expensive-predicate dispatch would have compilers defeat caching via specialization.
...
**Design-space contradiction**: Cold-start theorized expensive pre-JIT dispatch (1--2~µs) would enable caching. However, V8's pre-JIT (250~ns) is already optimized via interpreter-level inline caches, bytecode dispatch optimization, and branch prediction. At 250~ns, cache overhead (136~ns) dominates (1.55× slowdown). Modern interpreters optimize dispatch heavily before JIT, eliminating pre-JIT windows.

**Amortization**: Pre-JIT phase (<0.1\% long-running execution) is negligible. Even short-lived processes (100M calls) show <0.2~ns/call amortization—dwarfed by cache overhead (14--20~ns/call).
