;;; Test loading lsim-fol-simple.fol
...
(format t "~%=== Testing make-nbit-register-from-gates ===~%")
(let ((netlist (funcall (intern "MAKE-NBIT-REGISTER-FROM-GATES" :lsim-fol-test) 2)))
  (format t "Created netlist: ~A~%" netlist)
  (format t "Netlist type: ~A~%" (type-of netlist))
  (format t "Netlist size: ~D~%" (fol.compiler.collection-functions:size netlist)))
