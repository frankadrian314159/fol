...
(REGISTER-MODULE
 ...
 (MAKE '<MODULE-DEF> :NAME 'REGISTER-32BIT :PORTS
       (apply #'VECTOR
              (append '(CLK)
                      (cl:loop for i below 32 collect (cl:intern (cl:format nil "D~D" i) "LSIM"))
                      (cl:loop for i below 32 collect (cl:intern (cl:format nil "Q~D" i) "LSIM"))))
       :BODY
       (apply #'VECTOR
              (cl:loop for i below 32
                       collect (VECTOR 'D-LATCH
                                       (cl:intern (cl:format nil "BIT~D" i) "LSIM")
                                       :CLK 'CLK
                                       :D (cl:intern (cl:format nil "D~D" i) "LSIM")
                                       :Q (cl:intern (cl:format nil "Q~D" i) "LSIM")
                                       :QBAR (cl:intern (cl:format nil "QBAR~D" i) "LSIM"))))))
 ...
 (MAKE '<MODULE-DEF> :NAME 'TOP32X32 :PORTS (VECTOR)
       ...
       (apply #'VECTOR
              (cl:loop for stage below 32
                       collect
                       (let* ((reg-name (cl:intern (cl:format nil "REG~D" stage) "LSIM"))
                                ...
                                (cl:append
                                 ...
                                 (cl:loop for bit below 32
                                          append (cl:list
                                                  ...
                                                  (cl:intern (cl:format nil "~A~D" in-prefix bit) "LSIM")))
                                 (cl:loop for bit below 32
                                          append (cl:list
                                                  (cl:intern (cl:format nil "Q~D" bit) "KEYWORD")
                                                  (cl:intern (cl:format nil "~A~D" out-prefix bit) "LSIM"))))))))))

;;; Monitor final outputs Q0..Q31
(apply #'MONITOR
       (cl:loop for i below 32
                collect (cl:intern (cl:format nil "Q~D" i) "LSIM")))

;;; ADD-P1: even bits = 1, odd bits = 0
(DEFUN ADD-P1 (TIME-VAL)
  (apply #'ADDEVENTS
         (cl:loop for i below 32
                  collect (DICT :NODE (cl:intern (cl:format nil "D~D" i) "LSIM")
