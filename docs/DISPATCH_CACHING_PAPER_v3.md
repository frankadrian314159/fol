...
- **Conclusion**: Closure-capture issue is rare in practice when using proper development patterns (define methods before using predicates)
...
Completeness note: Classifier may reject some safe predicates (e.g., user-defined pure 
functions not annotated). This is intentional conservative design.
```
...
**Example: K=8, M=1000, B=20**
```
Coupon Collector: p ≤ 99.9%
Bursty bound: p ≥ 1 - 8·(1-(7/8)^20)/20 ≈ 96%
Real benchmark: 98.8% (exceeds predictions!)
```
...
Class D (K > 50, adversarial):        p ≤ 70%  → Caching not recommended
```
