...
allocations. On our benchmarks this allocation and its garbage collection make
up the bulk of the loop's runtime---the part this paper removes---while the loop
as a whole runs 90--250$\times$ slower than an equivalent native mutable-struct
loop, most of that gap being generic-arithmetic overhead we do not address.
Classic
...
byte-identical results and no cost when disabled; the residual distance, which
we measure and attribute to generic scalar arithmetic rather than allocation, is
the larger factor and orthogonal to this work.
\end{abstract}
...
Lisp---a loop that rebuilds a persistent record runs \textbf{90--250$\times$}
...
is, so removing allocation is necessary but not sufficient to reach native. We
attack it first deliberately: allocation is the factor that produces
garbage-collection pauses---which matter for latency, not merely
throughput---and it is the enabling step for the arithmetic work, since the
scalars it exposes are exactly what a later type-specialization pass would
annotate.
...
eliding it. The decomposition is consistent across all nine benchmarks: the
allocation factor ASR removes (its speedup) times the arithmetic factor it leaves
(its distance to native) equals the full baseline gap, and the arithmetic factor
is always the larger of the two. This also explains the speedup spread of RQ2:
the arithmetic-heavy kernels (Lorenz, Kalman, Mandelbrot) spend a smaller share
of their time allocating, so removing allocation buys proportionally less. The
residual is not allocation, of which none remains, but arithmetic: the unboxed
loop still calls FOL's \emph{generic} operators (\texttt{+}, \texttt{*},
\texttt{-}, \texttt{/}) on its scalar variables, whereas the native loop issues
declared single-float machine instructions. Closing it is a matter of scalar type
specialization, which is orthogonal to allocation and in fact complementary to
this transformation---scalar replacement is what exposes the per-field variables
that a type specializer would then annotate, and the two factors compose
multiplicatively toward native. We have prototyped this second factor as an
opt-in pass: it seeds each scalar's type from its literal initializer, propagates
it through the arithmetic to a fixpoint, and---wherever every operand is proven
numeric---lowers FOL's generic operators to native machine arithmetic on declared
floats (sound, since FOL's scalar operators equal the CL operators on numbers).
Applied on top of ASR, it delivers a \emph{further} 4.4$\times$--12.8$\times$ on
these benchmarks, largest on the arithmetic-heavy loops (Lorenz, Mandelbrot) that
gained least from allocation removal, and it moves the scalar-replaced loops from
4--6\% of native to roughly a quarter to a half of native speed---confirming the
multiplicative composition, and turning the residual from an open problem into a
complementary pass. Attacking allocation first is nonetheless not arbitrary: it
is the factor that produces garbage-collection pauses (the baseline triggers
21--47 collections per call; ASR triggers none), so it removes GC from the loop
entirely---a latency benefit, not merely the throughput factor the speedup
measures---and it is the prerequisite for the arithmetic work, since the
literal-seeded scalars ASR exposes are exactly what make the type inference
tractable.
...
and declines the rest. Across nine allocation-bound benchmarks it eliminates
per-iteration allocation entirely for 2.9$\times$--6.5$\times$ speedups with
identical results, recovering the allocation component of a 90--250$\times$ gap
to native mutable structs; the remainder---a larger factor we attribute to
generic scalar arithmetic---is closed by a complementary type-specialization
pass, which our prototype shows recovers a further 4.4$\times$--12.8$\times$. We
see the transformation as the record-shaped complement to automatic transient
