...
a whole runs 80--240$\times$ slower than an equivalent native mutable-struct
...
inapplicable. Across fourteen allocation-bound benchmarks the technique eliminates
per-iteration allocation entirely for an end-to-end 2.8$\times$--14.5$\times$
speedup (the top of that range from one dispatch-bound benchmark, \S\ref{sec:eval})---the
allocation share of the 80--240$\times$ gap to native---with
...
objects, transpiled to Common Lisp---a loop that rebuilds a persistent record runs \textbf{80--240$\times$}
...
\begin{enumerate}
  ...
  2.8$\times$--14.5$\times$ speedup (dominated by one dispatch-bound benchmark,
  \S\ref{sec:eval}; the rest span 2.8$\times$--6.4$\times$), produces byte-identical results, and adds no
...
loop-carried record accumulator---and we measure it on fourteen microbenchmarks that instantiate
...
The pass is implemented inside FOL's compiler in about 640 lines of Common Lisp:
roughly 350 in the core classify-and-rewrite walk (\S\ref{sec:loop}) that
...
numbers. The fourteen benchmarks span three families. The first eight are
physics-simulation kernels that exercise the core rewrite and its branch
...
fields under a coupled multiplicative map), a three-field \emph{ballistic}
integrator (the position update reads the freshly computed velocity), and a
coupled \emph{two-body} relaxation carrying \emph{two} \texttt{<vec2>}
accumulators that each read both---the multi-accumulator case. A \emph{clamp}
...
normal move)---the pass must classify and rewrite all three to accept the
loop, though this benchmark's trajectory only ever reaches the high wall and
the normal move; a \emph{phase} benchmark demonstrates \texttt{case}-branched
reconstruction where the iteration index modulo 3 deterministically visits
all three clauses; and an
\emph{assoc} benchmark demonstrates partial reconstruction from a generic
\texttt{assoc} update rather than a full constructor call. The next five are recognizable
...
\emph{Counter} benchmark is the simplest possible case: a single-field record
...
\textbf{RQ1: Firing and allocation.} The pass fires on all fourteen benchmarks: each
...
baseline conses one record (144--208\,B), or two (320\,B) in the
multi-accumulator cases---0.7--1.6\,GB per call, triggering 14--30 collections
costing 22--84\,ms---whereas the scalar-replaced version conses \emph{nothing}
...
\textbf{RQ2: Speedup and correctness.} Table~\ref{tab:speedup} reports results for all fourteen benchmarks.
...
2.8$\times$--14.5$\times$. One benchmark accounts for the top of that range on
its own. \emph{Assoc}'s update goes through FOL's generic \texttt{assoc}
dispatch rather than a specialized \texttt{make-<t>} constructor, so its
baseline is not merely allocation-bound but dispatch-bound: isolating the two
factors shows Assoc's unoptimized path costs \textasciitilde51\% more wall
time than an otherwise-identical full-reconstruction baseline over the same
four fields, on top of allocating slightly more per call (208\,B vs.\ 176\,B).
ASR removes both costs at once---the rewrite bypasses \texttt{assoc} entirely,
emitting direct scalar arithmetic---so Assoc reaches 14.5$\times$, more than
double any other benchmark.
Excluding it, the remaining thirteen span a more
typical 2.8$\times$--6.4$\times$, with \emph{Counter}---the simplest possible
single-field case---at the top of that range and \emph{Lorenz} at the bottom.
Within that span, the physics-family kernels (Particle,
Rotation, Ballistic, Two-body, Clamp, Bounce, Phase) see 3.7$\times$--6.0$\times$---\emph{Bounce}'s
extra \texttt{cond} tests on every iteration cost more than Clamp's single
\texttt{if}, making it the lowest of this group---while the arithmetic-heavier
kernels (Mandelbrot, Kalman, biquad, co-moments, Lorenz) see
2.8$\times$--4.1$\times$---the more a
loop computes per step, the smaller the share of its runtime that allocation
represents, and the smaller the win from removing it (RQ3). Crucially, every optimized program returns results
...
reconstructions, \texttt{if}/\texttt{cond}/\texttt{case}-branched
reconstructions, and multiple coupled accumulators), and the full compiler test
suite of 3{,}336 checks passes with the pass enabled.

\begin{table}
  \caption{Fourteen allocation-bound benchmarks at 5M iterations: wall time (mean
  ...
  drives per-iteration allocation (144--320\,B) to zero and is bit-identical to
  the baseline. The ``$\times$'' columns are speedups over the baseline,
  computed from full-precision per-trial timings before the Base/ASR/Native
  columns were rounded to whole milliseconds for display; dividing the
  rounded columns shown here will not always reproduce them exactly. The
  first eight are physics-family kernels exercising the core rewrite and its
  branch shapes (Clamp: \texttt{if}; Bounce: \texttt{cond}; Phase:
  \texttt{case}; Assoc: generic \texttt{assoc}); the next five are from other
  domains; Counter is the simplest possible single-field case. Two-body and
  Kalman carry two coupled record accumulators, both unboxed by the fixpoint.
  Assoc's outsized speedup comes from a dispatch-bound baseline, not an
  unusually fast optimized path (\S\ref{sec:eval}).}
  \label{tab:speedup}
  \small
  \begin{tabular}{lrrrrr}
    ...
    Benchmark & Base & ASR & Native & ASR & Native \\
              & (ms) & (ms) & (ms) & $\times$ & $\times$ \\
    \midrule
    Particle   & $3691\pm150$ & \phantom{0}$643\pm23$\phantom{0}  & \phantom{0}33 & \phantom{0}5.7 & 112 \\
    Rotation   & $5185\pm75$\phantom{0}  & $1003\pm13$ & \phantom{0}62 & \phantom{0}5.2 & \phantom{0}84 \\
    Ballistic  & $4750\pm85$\phantom{0}  & \phantom{0}$797\pm21$\phantom{0}  & \phantom{0}33 & \phantom{0}6.0 & 144 \\
    Two-body   & $12442\pm170$ & $2092\pm32$\phantom{0} & \phantom{0}52 & \phantom{0}5.9 & 240 \\
    Clamp      & $4583\pm48$\phantom{0}  & $1061\pm18$ & \phantom{0}35 & \phantom{0}4.3 & 130 \\
    Bounce     & $5253\pm180$ & $1404\pm12$ & \phantom{0}37 & \phantom{0}3.7 & 144 \\
    Phase      & $3657\pm113$ & \phantom{0}$665\pm17$\phantom{0}  & \phantom{0}24 & \phantom{0}5.5 & 150 \\
    Assoc      & $7505\pm160$ & \phantom{0}$518\pm22$\phantom{0}  & \phantom{0}33 & \textbf{14.5} & 229 \\
    \midrule
    Mandelbrot & $4344\pm67$\phantom{0}  & $1236\pm43$ & \phantom{0}52 & \phantom{0}3.5 & \phantom{0}84 \\
    Kalman     & $10913\pm163$ & $3769\pm155$ & 110 & \phantom{0}2.9 & \phantom{0}99 \\
    Biquad     & $6520\pm173$ & $1604\pm11$ & \phantom{0}56 & \phantom{0}4.1 & 116 \\
    Co-moments & $6668\pm84$\phantom{0}  & $1832\pm64$ & \phantom{0}71 & \phantom{0}3.6 & \phantom{0}94 \\
    Lorenz     & $6209\pm137$ & $2225\pm103$ & \phantom{0}64 & \phantom{0}2.8 & \phantom{0}97 \\
    \midrule
    Counter    & $2664\pm35$\phantom{0}  & \phantom{0}$414\pm16$\phantom{0}  & \phantom{0}33 & \phantom{0}6.4 & \phantom{0}81 \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{RQ3: Distance to native.} A hand-written native version---a mutable
\texttt{defstruct} updated in place---is 81$\times$--240$\times$ faster than the
...
consing, worth the 2.8$\times$--14.5$\times$ of RQ2. It does \emph{not} close the
gap entirely---the scalar-replaced loop remains 13$\times$--40$\times$ slower
than native, reaching 2--8\% of native speed. The decomposition is consistent
...
removal, and it moves the scalar-replaced loops from 2--8\% of native to
...
pauses (the baseline triggers 14--30 collections per call; ASR triggers none),
...
constructor that supplies \emph{every} field; \texttt{if}-, \texttt{cond}-, and
\texttt{case}-branched reconstructions are all recognized directly (Table~\ref{tab:speedup}
exercises all three: \emph{Clamp} the \texttt{if} case, \emph{Bounce} the
\texttt{cond} case, and \emph{Phase} the \texttt{case} case), but only when
each branch's constructor is reached without its own \texttt{bind}/\texttt{do}
peeling or inlinable-callee substitution---a branch that needs either aborts
the whole reconstruction. The remaining limitation is inlinable callees whose
...
because an unrecognized shape aborts.
...
pointer store}, which is atomic with respect to concurrent reads on any
architecture---a naturally-aligned word cannot be observed torn. We claim only
atomicity here, not ordering: the argument below needs no release-acquire
semantics, since the reader consults nothing but this one flag, so we make no
claim about weaker memory models beyond what atomicity alone provides. (All
measurements in this paper run on x86-64, where TSO gives ordering for free
in any case; we have not measured or verified the guard's behavior on ARM64
or other weakly-ordered architectures.) A concurrent reader therefore observes
either the pre- or the post-invalidation value---never a torn one---and,
because the transition is monotonic, a stale read is benign: at
...
records to classes that are not redefined mid-run, so it faces no version of
the redefinition problem our guard solves. Its JIT's partial escape analysis
\cite{stadler2014partial} could in principle eliminate the same allocation---but
only if the JIT's own inliner first brings the helper call into one compilation
unit, a heuristic decision we have not measured and that is not guaranteed for
an arbitrary loop-carried helper call reached through a back-edge. We do not
claim Clojure's JIT reliably eliminates this pattern in practice, only that,
unlike FOL, it has a plausible mechanism to attempt it and no redefinition
problem that would require a guard either way. A statically typed
...
validity: FOL is a research vehicle without an outside user base, and our fourteen
...
recognizes and declines the rest. Across fourteen allocation-bound benchmarks it
eliminates per-iteration allocation entirely for 2.8$\times$--14.5$\times$
speedups (one dispatch-bound benchmark accounts for the top of that range;
the rest span 2.8$\times$--6.4$\times$) with identical results, recovering the allocation component of a
80--240$\times$ gap to native mutable structs. The remainder---a larger factor
