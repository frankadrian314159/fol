...
Slot storage uses a hybrid strategy: objects with $\le 8$ slots use native CLOS slots, while wider objects use a 32-way persistent trie. Approximately 80\% of classes have $\le$8 slots~\cite{Lanza2006}; Table~\ref{tab:threshold-update} shows $T=8$ also minimizes modeled update cost for 8-slot objects. Redefinitions are handled lazily at the next mutation; renamed slots resolve via an alias map without triggering migration.

The \texttt{persistent-class} metaclass ensures post-initialization immutability; standard mutation paths raise errors. \texttt{assoc} returns a structurally equal ($=$) fresh instance sharing structure with the original. Snapshots decouple state from identity ($\textit{eq}$).

\subsection{Collections and Runtime}
FOL implements persistent \textbf{Vectors}, \textbf{Maps/Sets}, and \textbf{Sorted collections} using 32-way tries~\cite{Okasaki1998,Bagwell2000}. Branching factor 32 minimizes depth and optimizes updates. For keyword keys, HAMT lookup is $\approx$$1.3\times$ faster than \texttt{gethash} by avoiding EQL-path overhead. FOL transpiles to Common Lisp via expansion and code generation phases.
...
Collections and identity types (Atoms, Refs, Agents) hold only current values; old snapshots are GC-eligible unless referenced. Each \texttt{assoc} copies a root-to-leaf path ($O(\log_{32} N)$ nodes). SBCL nursery reclamation keeps GC overhead at 1.5\% in the LSim benchmark; the $14{\times}$ peak allocation in the AST benchmark (Section~\ref{sec:ast-bench}) reflects this volume but remains depth-bounded.
...
\end{lstlisting}
FOL reorders this to \texttt{even?} first, so even integers never reach \texttt{:any-integer}. User-defined predicates must maintain totality. Shadowed clauses are detected at definition or method-addition points; a continuable warning is signaled if a clause is dominated by a more-specific one.
...
We measured persistent objects vs.\ mutable CLOS across slot counts. \textbf{Construction} (${\approx}183$\,ns) is $5{\times}$ native; \textbf{Reads} add 1.2--2.0$\times$; \textbf{Updates} are 30--248$\times$ slower. Table~\ref{tab:threshold-update} show $T=8$ is optimal for coverage of the fully-native path, as wider classes are rare.
...
FOL single-hop first-touch cost is ${\approx}2{\times}$ CL's; multi-hop cost ($3.6$--$8.2\,\mu$s) does not grow linearly with depth, as chain-replay composes alias maps in a single pass (Tables~\ref{tab:schema-evo-single}--\ref{tab:schema-evo-multi}).
...
At scale, CL mutable heap fragmentation degrades locality under SBCL's default Windows allocation strategy; path-copying avoids this, yielding FOL's 3.8$\times$ advantage (averaged over three iterations for the largest configuration).
...
FOL matches Hybrid-A in all five ablated configurations: state-map (HAMT) allocation dominates the heap budget, so adding a persistent heap incurs negligible additional overhead when maps are already persistent.
At the extreme scale (32x32x32), Hybrid-A's persistent maps eliminate the 3.8$\times$ degradation seen in the CL baseline, while Hybrid-B (mutable maps) continues to suffer from fragmentation.
Both persistent-map and persistent-heap overheads peak at mid-scale (5--6$\times$) and fall at the extremes.
At the 32x32x32-30000 configuration, FOL ran 3.8$\times$ faster; the ablation in Table~\ref{tab:lsim-ablation} demonstrates that the gain is driven by persistent state maps avoiding hash-table fragmentation.
