...
\lstdefinelanguage{FOL}{
  morekeywords={defn,defgeneric,defmethod,defclass,defmacro,def,bind,if,cond,fn,lambda,let,do,loop,recur,throw,try,catch,quote,apply,map,reduce,filter,vec,str,make,assoc,get,merge,first,rest,nth,size,empty?,vals,update-vals,dotimes},
  sensitive=true,
  morecomment=[l]{;},
  morecomment=[s]{\#|}{|\#},
  morestring=[b]",
...
\begin{figure*}[htbp]
\centerline{\includegraphics[width=\textwidth]{ELS-2026-paper-class-hierarchy.pdf}}
\caption{FOL Type Hierarchy showing inheritance from \texttt{<persistent-object>}.}
\label{fig:hierarchy}
\end{figure*}
...
\begin{table}[htbp]
\caption{Implementation File Summary}
\begin{center}
\begin{tabular}{|l|c|l|}
\hline
...
FOL extends CLOS with a persistent object protocol. All user-defined classes inherit from \texttt{<persistent-object>} and use \texttt{pslot-value} for slot access (although, as in the code below, one can also use the function \texttt{assoc} to assign new values, access values via a keyword having the slot-name, or call the \texttt{get} function to retrieve the value of a slot):
...
In addition, one can specify a set of destructuring patterns in \texttt{defgeneric} and a set of patterns with their associated code in functions, methods and macros:
...
\begin{table}[htbp]
\caption{Complexity Comparison: Persistent vs. Mutable}
\begin{center}
\begin{tabular}{|l|c|c|}
\hline
...
\hline
Multi-pattern dispatch & $\checkmark$ & $\checkmark$ & $\times$ \\
\hline
...
Clojure \cite{b4} pioneered persistent data structures in production Lisp. FOL adopts Clojure's sequence abstraction, transducer protocol and persistence while diverging in object system design. Where Clojure uses protocols and records, FOL uses CLOS generic functions and classes.

\subsection{Common Lisp}

FOL's object system is a direct descendant of CLOS \cite{b2}. The meta-object protocol \cite{b3} enables deep customization of class and generic function behavior. FOL extends this with persistent slot values.

\subsection{Dylan}

Dylan \cite{b5} influenced FOL's naming conventions and multiple dispatch semantics. The \texttt{<type>} notation improves code readability by clearly distinguishing types. In addition, FOL adopts Dylan's module system. Dylan modules provide a persistent way to package symbols. We believe it is also simpler and more consistent when compared with Clojure's namespaces and Common Lisp's packages.
...
\bibitem{b1} G. L. Steele Jr., \emph{Common LISP: The Language}, 2nd ed. Digital Press, 1990.

\bibitem{b2} D. G. Bobrow, L. G. DeMichiel, R. P. Gabriel, S. E. Keene, G. Kiczales, and D. A. Moon, ``Common Lisp Object System Specification,'' \emph{ACM SIGPLAN Notices}, vol. 23, no. SI, pp. 1--142, 1988.

\bibitem{b3} G. Kiczales, J. des Rivi\`{e}res, and D. G. Bobrow, \emph{The Art of the Metaobject Protocol}. MIT Press, 1991.

\bibitem{b4} R. Hickey, ``The Clojure programming language,'' in \emph{Proceedings of the 2008 Symposium on Dynamic Languages}, 2008.

\bibitem{b5} Apple Computer, Inc., \emph{Dylan Interim Reference Manual}. Apple Computer, Inc., 1993.

\bibitem{b6} R. Hickey, ``Transducers,'' \emph{Clojure Blog}, August 2014. Available: https://clojure.org/reference/transducers

\bibitem{b7} C. Okasaki, \emph{Purely Functional Data Structures}. Cambridge University Press, 1999.

\bibitem{b8} S. L. Siskind, ``FSet: A functional set-theoretic collections library for Common Lisp,'' \emph{Quicklisp}, 2012. Available: https://common-lisp.net/project/fset/
\end{thebibliography}
