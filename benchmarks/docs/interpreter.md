...
|---|---|---|---|
| CL (mutable CLOS) | 9.6 ms | 3.81 MB | 1.0× |
| FOL (persistent objects) | 19.9 ms | 8.87 MB | 2.1× time, 2.3× memory |

### Eval Phase (1,000 iterations × 50 exprs × 3 generic functions)

| Implementation | Total time | Alloc | Time/expr | vs. CL |
|---|---|---|---|---|
| CL (CLOS dispatch + hash-table env) | 0.269 s | 224.5 MB | 5.38 µs | 1.0× |
| FOL (predicate dispatch + persistent dict) | 1.879 s | 1300.0 MB | 37.57 µs | 7.0× time, 5.8× memory |
...
The AST-optimizer ratio (~47×) represents the worst case: `defstruct` is the
...
Measured overhead: **7.0× time, 5.8× memory** — substantially better than the
AST-optimizer worst-case (~47×) because the CL baseline is standard CLOS
