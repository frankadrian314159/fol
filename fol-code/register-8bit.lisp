...
(REGISTER-PRIMITIVE 'D-LATCH
 (LAMBDA (NAME PARAM CONNS)
   (MAKE <LOGIC-COMPONENT> :NAME NAME :TYPE 'D-LATCH :CONNECTIONS CONNS :INPUTS
    (FOL.COMPILER.COLLECTIONS:SET :CLK :D) :OUTPUTS
    (FOL.COMPILER.COLLECTIONS:SET :Q) :DELAYS
    (FOL.COMPILER.COLLECTIONS:DICT :CLK (FOL.COMPILER.COLLECTIONS:DICT :Q 3) :D
                                   (FOL.COMPILER.COLLECTIONS:DICT :Q 4))
    :LOGIC-FN
    (LAMBDA (S)
      (LET ((D (GET S :D)))
        (LET ((CLK (GET S :CLK)))
          (LET ((PREV-Q (GET S :PREV-Q NIL)))
            (FOL.COMPILER.COLLECTIONS:DICT :PREV-Q
...
(DEFPART REGISTER-8BIT
 (FOL.COMPILER.COLLECTIONS:VECTOR D0 D1 D2 D3 D4 D5 D6 D7 CLK Q0 Q1 Q2 Q3 Q4 Q5
                                  Q6 Q7)
...
(DEFPART TEST-REGISTER-8BIT
 (FOL.COMPILER.COLLECTIONS:VECTOR CLK IN0 IN1 IN2 IN3 IN4 IN5 IN6 IN7)
