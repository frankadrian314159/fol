...
;;;
;;; Provides immutable objects with structural sharing via FSet wb-maps.
;;; All FOL user-level classes inherit from <persistent-object>, storing
;;; slot values in an FSet map rather than standard CLOS slots.
;;;
...
;;;
;;; Keys are stored as keyword symbols for fast eq-based lookup.
...
(defclass <persistent-object> (standard-object)
  ((%persistent-storage :accessor %persistent-storage
                        :documentation "FSet map storing all slot values."))
...
(defun slot-key (slot-name)
  "Convert a slot name symbol to a keyword for FSet map lookup."
...
(defmethod initialize-instance :after ((object <persistent-object>) &rest initargs)
  "Populate the FSet map from initargs and initforms."
        ...
        (unless (eq slot-name '%persistent-storage)
          (cond
            ((not (eq init-value :not-found))
             (setf storage (fset:with storage keyword init-value)))
...
(defmethod closer-mop:slot-boundp-using-class ((class persistent-class)
                                               object
                                               (slot closer-mop:standard-effective-slot-definition))
  "Check if slot exists in FSet map."
  (let ((slot-name (closer-mop:slot-definition-name slot)))
    (if (eq slot-name '%persistent-storage)
        (call-next-method)
        (if (slot-boundp object '%persistent-storage)
            (nth-value 1 (fset:lookup (%persistent-storage object)
                                      (slot-key slot-name)))
            (call-next-method)))))
...
(defun update-slot (object slot-name new-value)
  "Return a new persistent object with SLOT-NAME updated to NEW-VALUE.
   Shares structure with OBJECT via FSet's persistent map."
  (let* ((class (class-of object))
         (new-obj (allocate-instance class))
         (old-storage (%persistent-storage object))
         (new-storage (fset:with old-storage (slot-key slot-name) new-value)))
    (let ((*initializing-persistent-object* t))
...
(defun update-slots (object &rest slot-name-value-pairs)
  ...
  (let* ((class (class-of object))
    ...
    (loop for (slot-name value) on slot-name-value-pairs by #'cddr
          do (setf storage (fset:with storage (slot-key slot-name) value)))
    (let ((*initializing-persistent-object* t))
