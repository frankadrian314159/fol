...
\begin{enumerate}
  ...
  and division-bearing updates, and two coupled multi-accumulator loops, the
...
numbers. The nine benchmarks span two families. The first four are
...
each read both---the multi-accumulator case. The other five are recognizable
...
temporary-free Euler step so the accumulator is the only allocation).
...
\textbf{RQ2: Speedup and correctness.} Table~\ref{tab:speedup} reports results
for all nine benchmarks. Eliminating allocation removes the garbage-collection
pressure that dominates each baseline, yielding end-to-end speedups of
2.9$\times$--6.5$\times$. The spread is itself informative: the physics kernels,
...
removing it (RQ4). Crucially, every optimized program returns results
...
\begin{table}
  \caption{Nine allocation-bound benchmarks at 5M iterations: wall time (mean
  ...
  first four are physics kernels; the last five are from other domains. Two-body
...
the \emph{allocation} component of that gap: it removes 100\% of per-iteration
